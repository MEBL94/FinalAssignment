...
namespace FinalAssignment.Services
{
    public class EmailService : IEmailService
    {
        ...
        public List<EmailAddress.EmailMessage> ReceiveEmail(int maxCount = 10)
        {
            using (var emailClient = new Pop3Client())
            {
                ...
                List<EmailAddress.EmailMessage> emails = new List<EmailAddress.EmailMessage>();
                for (int i = 0; i < emailClient.Count && i < maxCount; i++)
                {
                    var message = emailClient.GetMessage(i);
                    var emailMessage = new EmailAddress.EmailMessage
                    {
        ...
        public void Send(EmailAddress.EmailMessage emailMessage)
        {
